...
const HACKER_TOTAL_MS      = 3000; // медленнее печать
...
const hackLog       = document.querySelector('.hack-log');
const resultsTitle  = document.querySelector('.results-title');
const resultsList   = document.querySelector('.results-list');

// кнопки языков
const langEmoji = document.querySelector('.lang-emoji');
...
// ===== Переводы =====
const TRANSLATIONS = {
  en: { getStart: "START",    crushLabel: "CRUSH",  resultSuffix: "x", results: "Results" },
  ru: { getStart: "СТАРТ",    crushLabel: "КРАШ",   resultSuffix: "x", results: "Результаты" },
  es: { getStart: "INICIAR",  crushLabel: "CRASH",  resultSuffix: "x", results: "Resultados" }
};
let currentLang = "en";

// ===== Хакерское меню (всегда английский) =====
const hackerLines = [
...
async function runHackerConsole(){
  ...
  let baseDelay = Math.max(5, Math.round(HACKER_TOTAL_MS / totalChars));
  baseDelay = clamp(baseDelay, 5, 40);
...
async function showResultAt(x,y,result){
  ...
  const label = (result.type==='CRUSH') ? t.crushLabel : `${result.value}${t.resultSuffix}`;
...
function renderResultsList(){
  ...
  const t = TRANSLATIONS[currentLang];
  resultsHistory.slice(0, 3).forEach(r => {
    const row = document.createElement('div');
    const val = (r.type === 'CRUSH') ? t.crushLabel : `${r.value}${t.resultSuffix}`;
...
// ===== Меню языков =====
if (langEmoji){
  langEmoji.addEventListener('click', (e) => {
    e.stopPropagation();
...
document.addEventListener('click', (e) => {
  ...
  if (e.target === langEmoji || langMenu.contains(e.target)) return;
  langMenu.hidden = true;
});
...
async function handleLaunch(){
  ...
  try {
    ...
    if (first){
      ...
      try { sessionStorage.setItem(HACKER_SHOWN_KEY, '1'); } catch(e){}
    }
